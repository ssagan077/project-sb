...
function send_request() {
  ...
  out_dir="${remote_exch_dir}/client"
  echo "Sending request to CA server..."
  scp ${req_dir}/${cli_file} ${USER}@${ca_address}:${out_dir}/ && echo "Client request has been sent"
}
...
#Checking connection
echo "Checking connection with CA server..."
echo "test" > test
scp test ${USER}@${ca_address}:/tmp/ && echo "Ok"
if [ $? -ne 0 ]; then
   echo "Problem with uploading files to CA server. "
...
send_request
if [ $? -ne 0 ]; then echo "Can't send client request. Probably CA server doesn't have our public key"
