...
## This action is necessary to exchange data with the CA server through the exchange folders hosted on the vm-pckg-srv server.
...
if [[ $REPLY =~ ^[Yy]$ ]]
      then
      ssh-copy-id $USER@$pckg_address && echo -e "\nssh keys have copied to PCKG-server"
      if [ $? -ne 0 ]; then
...
echo "Done. Now we can create requests to PCKG server"
