...
pckg_dir="${packages_dir}/synch-git-0.1"
data_dir="${git_dir}/source/synch-git"
...
ls synch-git_* 2>/dev/null && echo "Package synch-git already exist. Please, remove it manually to continue setup" && exit 1

#Copy config files
echo "Copying config files..."

if [ ! -f ${data_dir}/synch-git_list ]; then
   echo "File ${data_dir}/synch-git_list not found!"
   exit 1
fi
...
echo "synch_git.sh usr/bin/" >> debian/install
echo "synch-git.service etc/systemd/system/" >> debian/install
echo "synch-git.timer etc/systemd/system/" >> debian/install

#Copy config files
[ -f $data_dir/preinst ] && cp $data_dir/preinst debian/
[ -f $data_dir/postinst ] && cp $data_dir/postinst debian/
[ -f $data_dir/prerm ] && cp $data_dir/prerm debian/
[ -f $data_dir/postrm ] && cp $data_dir/postrm debian/
