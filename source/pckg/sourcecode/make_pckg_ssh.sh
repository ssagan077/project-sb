...
pckg_dir="/home/$USER/packages/deb-ssh-tmp-0.1"
data_dir="/home/$USER/data/ssh"
...
ls deb-ssh_* 2>/dev/null && echo "Package deb-pckgs already exist. Please, remove it manually to continue setup" && exit 1

#Copy config files
echo "Copying config files..."

if [ ! -f ${data_dir}/sshd_config_tmp ]; then
   echo "File $data_dir/sshd_config not found!"
   exit 1
else
